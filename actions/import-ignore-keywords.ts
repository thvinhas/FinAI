...
export async function getIgnoreKeywords(): Promise<string[]> {
...
export async function addIgnoreKeyword(keyword: string) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
...
export async function removeIgnoreKeyword(keyword: string) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { error: "Não autenticado" }
