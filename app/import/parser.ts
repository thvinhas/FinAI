...
export function filterGarbage(
  ...
  if (ignoreKeywords.length === 0 && DEFAULT_IGNORE_KEYWORDS.length === 0) return transactions
  const allKeywords = [...new Set([...DEFAULT_IGNORE_KEYWORDS, ...ignoreKeywords])]
  return transactions.filter((t) => {
    const lower = t.description.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    return !allKeywords.some((k) => {
      const words = lower.split(/\s+/)
      return words.some((w) => w === k) || lower.includes(k)
    })
  })
}
...
export function extractGarbageKeywords(
  ...
  const allKeywords = [...new Set([...DEFAULT_IGNORE_KEYWORDS, ...currentIgnoreKeywords])]
...
const DEFAULT_TRANSFER_KEYWORDS = [
  "ted", "pix", "doc", "transferencia", "transf", "transferência",
  "transferencia enviada", "transferencia recebida",
  "ted enviada", "ted recebida", "pix enviado", "pix recebido",
...
function classifyType(isReceita: boolean, transfer?: boolean): ParsedTransaction["type"] {
...
export function detectFormat(fileName: string, text: string): "csv" | "ofx" {
  ...
  if (text.trim().startsWith("OFXHEADER") || text.includes("<OFX>")) return "ofx"
  return "csv"
}
...
export function parseCSV(
  ...
  const result = rows
    .map((row) => {
      ...
      const desc = (row[headers[col.description]] ?? "").trim()
...
export function parseOFX(text: string, transferKeywords?: string[]): ParsedTransaction[] {
